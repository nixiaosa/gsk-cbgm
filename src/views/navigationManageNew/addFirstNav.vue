    ...
    <div>
      <el-form :model="form">
        <el-form-item label="一级栏目名称:">
          <el-input :maxlength="6" v-model="form.name" auto-complete="off" style="width:200px;"></el-input>
        </el-form-item>
        ...
        <el-form-item label="是否添加二级栏目：">
          <el-radio-group v-model="form.havingChild" @change="getExamineValue()" :disabled="isDisabled">
            <el-radio :label="1">是</el-radio>
            <el-radio :label="0">否</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="一级栏目类型:" v-if="form.havingChild == 0">
          <el-select v-model="value" placeholder="请选择" @change="selectOne" :disabled="isDisabled">
            <div v-for="item in existBusiness" :key="item.id">
              <el-option :label="item.id | getType" :value="item.id"></el-option>
            </div>
          </el-select>
        </el-form-item>
        <el-form-item label="选择风格:" v-if="form.havingChild == 0 && value != 8 && styleIsShow">
        ...
        <el-form-item label="外链地址:" v-if="form.havingChild == 0 && value == 8">
          <el-input v-model="outerUrl" auto-complete="off" style="width:200px;"></el-input>
        </el-form-item>
        <el-form-item label="打开方式：" v-if="form.havingChild == 0 && value == 8">
...
export default {
  ...
  methods: {
    ...
    getStyleIn: async function() {
      var res = await http.get(api.getStyle + "/" + this.value);
      if (res.data.code === 0) {
    ...
    getParentsEdit: async function(page) {
      //一级栏目编辑页回显
      var res = await http.get(api.parentsList + "/" + this.$route.query.id);
      if (res.data.code === 0) {
        this.form.name = res.data.data.name;
        this.form.showOrder = res.data.data.showOrder;
        this.form.havingChild = res.data.data.havingChild;
        this.havingChild = res.data.data.havingChild;
    ...
    getParentList: async function(page) {
      ...
      if (res.data.code === 0) {
        ...
        // this.existBusiness = res.data.data.list;
      } else {
    ...
    createMenu: async function() {
      if (!this.form.havingChild && this.value != 8) {
        // 添加二级栏目为否且栏目类型非外链
        if (!this.form.name) {
      ...
      } else if (!this.form.havingChild && this.value == 8) {
        // 添加二级栏目为否且栏目类型是外链
        if (!this.form.name) {
          this.errorTost("请添加一级栏目名称");
          return false;
      ...
      if (!this.$route.query.id) {
        ...
        // if (this.havingChild == 1) {
        //   this.errorTost("请先停用该一级栏目下二级栏目，再设置该一级栏目类型");
        //   return false;
        // }
