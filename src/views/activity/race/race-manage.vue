  ...
  <div class="case">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item label="竞赛名称">
        <el-input v-model="formInline.name" placeholder="请输入竞赛名称"></el-input>
      </el-form-item>
      <el-form-item style="">
    ...
    <el-table
      ...
      border
    >
      <el-table-column label="竞赛名称" width="300px">
        <template slot-scope="scope">
          <span>{{ scope.row.examPaperName }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="每日答题次数">
        <template slot-scope="scope">
          <span>{{ scope.row.limitedNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="每次答题限时">
        <template slot-scope="scope">
      ...
      <el-table-column label="已答题数">
        <template slot-scope="scope">
          <span style="color: rgb(204,0,153)">{{ scope.row.totalNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已答对题数">
        <template slot-scope="scope">
          <span style="color: rgb(204,0,153)">{{ scope.row.rightNum }}</span>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      findExamManageLis: async function(pageNum) {
        ...
        const res = await http.post(api.raceList,params)
        if (res.data.code === 0) {
