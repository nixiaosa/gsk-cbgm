   ...
   <div class="participants">
     ...
     <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-table-column label="答题用时">
        <template slot-scope="scope">
          <span>{{ scope.row.useTime | tomin }}</span>
        </template>
      </el-table-column>
      <el-table-column label="本次答对数">
        <template slot-scope="scope">
          <span>{{ scope.row.rightNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="本次答题数">
        <template slot-scope="scope">
...
<script>
  ...
  import userId from "@/views/activity/race/base/identity.json";
  export default {
    name: "daystatistics",
    props:['id','page'],
    components: {
    ...
    methods: {
      answerExport2: async function(pageNum) {
        ...
        const res = await http.post(api.dayList,params)
        if (res.data.code === 0) {
      ...
      async answerExport() {
        ...
        await http.post(api.dayExport,params).then(res => {
          const data = res.data
          if (data.code !== 0) {
      ...
      participants: async function(pageNum) {
        ...
        const res = await http.post(api.dayList,params)
        if (res.data.code === 0) {
          this.totalUser = res.data.data.result.count
