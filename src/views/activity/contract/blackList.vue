...
export default {
  ...
  methods: {
    ...
    batchAdd: async function(fileUrl,name) {
      ...
      const res = await http.post(api.contractBlacklistImport,params);
      if (res.data.code === 0) {
    ...
    remove: async function(id) {
      ...
      var res = await http.post(api.contractBlackListRemove, params);
      if (res.data.code === 0) {
        this.getBlackList(1)
      } else {
        this.$message.error(res.data.message);
    ...
    getBlackList: async function(pageNum){
      ...
      const res = await http.post(api.contractBlacklist, params);
      if (res.data.code === 0) {
