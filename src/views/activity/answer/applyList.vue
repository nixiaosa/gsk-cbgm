...
import applyNotIdentityJson from "@/views/activity/answer/base/applyNotIdentity";
import applyHasIdentityJson from "@/views/activity/answer/base/applyHasIdentity";
...
export default {
  ...
  methods: {
    ...
    async downLoad(){
      ...
      const res = await http.post(api.answerApplyList,params);
      if (res.data.code === 0) {
    ...
    applyPassBatch: async function(row){
      ...
      const res = await http.post(api.answerApplyAudit, params);
      if (res.data.code === 0) {
        this.getApplyList(1)
