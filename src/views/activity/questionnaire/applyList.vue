...
import applyNotIdentityJson from "@/views/activity/questionnaire/components/applyNotIdentity";
import applyHasIdentityJson from "@/views/activity/questionnaire/components/applyHasIdentity";
...
export default {
  ...
  methods: {
    ...
    async downLoad(){
      ...
      const res = await http.post(api.questionnaireApplyList,params);
      if (res.data.code === 0) {
    ...
    applyPassBatch: async function(row){
      ...
      const res = await http.post(api.questionnaireApplyAudit, params);
      if (res.data.code === 0) {
        this.getApplyList(1)
