<template>
  <div id="CreateContent">
    <div v-for="(items,index) in addoptions.conditions" id="Receive">
      <div class="close" @click="close(items,index)" v-if="!id">x</div>
      <div class="close" @click="closes(items,index)" v-if="id">x</div>
      <el-form>
        <el-form-item v-for="item in itemData" :key="item.id" :label="item.label" class="formStyle" prop="name">
          <select-tree width="430" :options="options" :props="defaultProps" @selecteds="selecteds(index,arguments)" v-if="item.label=='领取身份'" v-model="items.identity" />
          ...
          <el-select placeholder="请选择 " style="width: 430px; " v-if="item.label=='观看时长'" v-model="items.pro.browseTime">
            <el-option v-for="(item,index) in browseTime " :key="index " :label="item.label " :value="item.time " style="padding: 10px;">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <div class="totals" v-if="addoptions.pointType == 1">总积分:{{items.pointMax}}积分</div>
      <div class="totals" v-if="addoptions.pointType == 0">总医币:{{items.pointMax}}医币</div>
    </div>
  </div>
</template>
<script>
...
<style scoped>
  .formStyle {
    width: 50%;float:left;height:60px;margin-top:30px;
