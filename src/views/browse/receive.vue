<template>
  <div id="list">
    ...
    <div id="right">
      <el-button size="mini" type="info" @click="openDialog">导出领取结果</el-button>
      <el-dialog
        title="提示"
    ...
    <el-table :data="tableData" border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="关联内容" width="230">
        <template slot-scope="scope">
          {{ scope.row.busName }}
        </template>
      </el-table-column>
  ...
  export default {
     ...
     * */
      ...
      getXls: async function() {
        var res = await http.get(api.lookWatch + this.taskId)
        if (res.data.code === 0) {
     ...
     lookReceive: async function(page) {
        var params = {
          pageNum: page,
          pageSize: 10,
          params:{
        ...
        var res = await http.post(api.lookReceive,params)
        if (res.data.code === 0) {
