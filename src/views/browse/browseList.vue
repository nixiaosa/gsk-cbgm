<template>
  <div id="list">
    <header-content :opertions="opertions" @header-btn-click="headerClick" :title="headerTitle" v-if="userAuths.create_video_look"></header-content>
    <header-content :title="headerTitle" v-if="!userAuths.create_video_look" :tableData="tableData" :total="total" @searchData="searchData"></header-content>
    <Headers v-on:change="setForm" :itemDatas="itemDatas">
    </Headers>
...
import Headers from '@/components/search/index'
export default {
  name: 'sharetask',
  components: {
    HeaderContent,
  ...
  methods: {
    ...
    headerClick() {
      this.$router.push({ path: '/taskcenter/creates' })
    },
    gteinviteList: async function(page) {
      ...
      var res = await http.post(api.lookSearch, params)
      if (res.data.code === 0) {
    ...
    handleClick(id) {
      ...
      this.$router.push({ path: 'lookdetails' + '/' + page + '?id=' + id })
    },
    handleClicks(id) {
    ...
    receive(id) {
      let page = this.newpage
      this.$router.push({ path: 'receives' + '/' + page + '?id=' + id })
    }
  }
}
</script>
