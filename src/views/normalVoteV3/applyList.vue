...
import applyNotIdentityJson from "@/views/normalVoteV3/base/applyNotIdentity";
import applyHasIdentityJson from "@/views/normalVoteV3/base/applyHasIdentity";
...
export default {
  ...
  methods: {
    ...
    async downLoad(){
      ...
      const res = await http.post(api.voteApplicationApplyListExport,params);
      if (res.data.code === 0) {
    ...
    applyPassBatch: async function(row){
      const params = {
        voteApplyIds: this.multipleSelection,
        status: 1
      };
      const res = await http.post(api.voteApplyOperate, params);
      if (res.data.code === 0) {
        this.getApplyList(1)
    ...
    refuse: async function(isBatchRefuse){
      ...
      if(!isBatchRefuse){ // 单个拒绝
        const params = {
        voteApplyId: this.refuseId,
        status: 2,
        rejectReason: this.reasonMessage
      };
      const res = await http.post(api.voteApplyOperate, params);
        if (res.data.code === 0) {
          this.getApplyList(1)
    ...
    getApplyList: async function(pageNum){
      ...
      if( this.identity ){
        params.param.identity = this.identityTit
      }
      const res = await http.post(api.voteApplicationApplyList, params);
      if (res.data.code === 0) {
