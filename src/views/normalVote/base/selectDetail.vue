<template>
  <div class="casedetails">
    ...
    <div class="wrap">
      ...
      <span class="votecode">{{ this.voteCode }}</span>
    </div>
    <div class="wrap" v-if="photoStatus == 1" style="position:relative; height: 100px">
      <span class="titls" style="width: 80px;">头像:</span>
      <img :src="this.headImg" alt width="100" height="100" style="position: absolute;top: 0px;">
    </div>
    <div class="wrap">
      <span>
        选项名称:
      ...
      <div class="userBoxRightList">
        <ul>
          <li v-for="item in attachList">
            <span>{{item.name}}</span>
            <span @click="goPFill(item.url)" class="listBtn">查看</span>
          </li>
        </ul>
      </div>
    </div>
  ...
  methods: {
    ...
    caseActivityDetail: async function() {
      ...
      if (res.data.code === 0) {
        ...
        // this.imgUrl = res.data.data.voteOption.imgUrl;
        this.headImg = res.data.data.voteOption.imgUrl;
        this.optionName = res.data.data.voteOption.optionName;
        this.description = res.data.data.voteOption.description;
        let fileUrlArr = JSON.parse(res.data.data.voteOption.fileUrl);
