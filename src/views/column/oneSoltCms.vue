<template>
  <div class="statis-area">
    ...
    <el-table :data="tableListData" style="width: 100%">
      ...
      <el-table-column label="IP归属地">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.ipRegion }}</span>
        </template>
      </el-table-column>
      <el-table-column label="浏览时长(分)">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.durationCount | millisToMinutesAndSeconds }}</span>
        </template>
      </el-table-column>
      <el-table-column label="访问次数">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.viewCounts }}</span>
        </template>
      </el-table-column>
      <el-table-column label="首次进入时间">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.firstViewTime | formatDateMinute}}</span>
        </template>
      </el-table-column>
      <el-table-column label="最后在线时间">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.lastViewTime | formatDateMinute}}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      formInline: {
        currentUri: "https://" + window.location.host + "/#/column/article" + "/" + this.$route.query.columnId,
        mobileCurrentUri: "https://" + window.location.host + "/mobile/#/column/article" + "/" + this.$route.query.columnId,
        // currentUri: "http://" + "localhost:8090" + "/#/column/article" + "/" + this.$route.query.columnId,
        element: "article",
        intervalType: "DAY",
        loginStatus: "ALL",
      },
  ...
  methods: {
    ...
    exportList() {
      ...
      this.$http.post(api.oneSoltExcel, params).then(res => {
    ...
    dataList: async function(pageNum) {
      ...
      params.pageSize = 10;
      let res = await http.post(api.oneSoltViewRecords, params);
    ...
    getPvChart: async function() {
      ...
      //   this.xAxis[index].push(formatDate(new Date(item.key), "yyyy-MM-dd"))
      //   this.drawLine();
      // })
