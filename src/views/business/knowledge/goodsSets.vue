  ...
  <div class="modelist goodsSets">
    ...
    <el-form :model="form" label-width="100px" :label-position="labelPosition" ref="artForm">
      <!-- <el-form-item label="内容防复制:" prop="title" class="switch">
        ...
        :inactive-value= "1"
        ></el-switch>
        <span v-if="!form.isCopy">允许复制</span>
        <span v-if="form.isCopy">禁止复制</span>
        <p v-if="!form.isCopy">商品的文字内容允许复制，图片点击放大和长按识别二维码功能允许使用</p>
        <p v-if="form.isCopy">商品的文字内容禁止复制，图片点击放大和长按识别二维码功能不允许使用</p>
      </el-form-item> -->
      <el-form-item label="售卖方式:" prop="keyword">
        <el-radio v-model="form.fee" :label="0">免费</el-radio>
        <el-radio v-model="form.fee" :label="1">付费</el-radio>
      </el-form-item>
      <el-form-item v-show="form.fee"  label="试看内容:" prop="keyword">
        <div class="quill-editor-example">
          <!-- quill-editor -->
          <quill-editor v-model="form.freeContent" ref="myTextEditor" :options="editorOption"></quill-editor>
        </div>
      </el-form-item>
      <el-form-item v-if="form.fee" label="商品价格:" prop="keyword" class="star">
        <el-input v-model.trim="form.unitPrice" style="width:300px" type="number" @blur="rulePrice">
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item v-if="form.fee" label="划线价格:" prop="keyword">
        <el-input v-model.trim="form.lineationPrice" style="width:300px" type="number" oninput="value=value.replace(/[^0-9.]/g,'')">
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item v-if="form.fee" label="商品有效期:" prop="keyword" class="star">
        <el-input v-model.trim="expireTimeCp" style="width:300px" class="input-with-select" type="number" oninput="value=value.replace(/[^0-9.]/g,'')">
          <el-select v-model="dataSelect" slot="append" placeholder="请选择" @change="selectChange">
            <el-option
      ...
      <el-form-item v-if="form.fee === 1 && startTime" label="商品设置时间:" prop="keyword">
        <el-input v-model="startTime" style="width:300px" disabled></el-input>
      </el-form-item>
    </el-form>
    <el-upload
      id="contentimg"
...
export default {
  data() {
    ...
    return {
      ...
      dataColumnId: "",
      form: {
        itemUnitNo: +this.$route.query.id,
        itemUnitName: this.$route.query.title,
        type: 17,
        // isCopy: 0,
        fee: 0,
        freeContent: null,
        unitPrice: null,
        expireTime: null,
        coverUrl: this.$route.query.imageUrl,
      },
      startTime:"",
      editorOption: {
        placeholder: "请输入内容",
  ...
  mounted() {
    this.dataSelect = this.selectoptions[0].value;
    this.payArticleGetInfo()
    var vm = this;
  ...
  methods: {
    ...
    rulePrice(){
      if(this.form.unitPrice < 0.01){
        this.$message.error("商品价格最低为0.01元");
        this.form.unitPrice = 0.01
      }
    },
    ...
    if(this.form.fee === 1){
      ...
      if(!this.expireTimeCp){
        this.$message.error("商品有效期不能为空");
        return false;
      }
      if (Number(this.expireTimeCp) < 1) {
        this.$message.error('商品有效期必须大于0！')
        return false
      }
      if (!Number.isInteger(Number(this.expireTimeCp))) {
        this.$message.error('请输入正确的商品有效期！')
        return false
      }
    }
      ...
      if(this.form.lineationPrice === ''){
        params.lineationPrice = 0
      }
      params.startTime = undefined
      params.expireTime = this.form.expireTime = this.expireTimeCp + this.dataSelect
      var res = await http.post(api.payArticleEdit, params);
      if (res.data.code === 0) {
    ...
    payArticleGetInfo: async function() {
      var res = await http.get(api.payArticleGetInfo + 17 + '/' + this.form.itemUnitNo);
      if (res.data.code === 0 && res.data.data) {
        if(res.data.data.id) {
          this.form = res.data.data
          if(this.form.lineationPrice === 0){
            this.form.lineationPrice = ''
          }
          this.startTime = this.formatDate(res.data.data.startTime)
          this.form.id = res.data.data.id
          this.dataSelect = res.data.data.expireTime.replace(/^(.*[n])*.*(.|n)$/g, "$2")
          this.expireTimeCp = res.data.data.expireTime.substring(0,res.data.data.expireTime.length-1)
        }
      }
    },
    selectChange(val){
      this.form.expireTime = this.expireTimeCp + val
      this.dataSelect = val
    },
