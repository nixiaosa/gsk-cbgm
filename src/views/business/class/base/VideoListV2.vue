<template>
  <div>
    <el-table :data="tableData" border>
      ...
      <el-table-column label="一级栏目" v-if="businessId!=='3'">
        <template slot-scope="scope">
          <span>{{ scope.row.firstColumnName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="二级栏目" v-if="businessId!=='3'">
        <template slot-scope="scope">
        ...
        <template slot-scope="scope">
          <div class="active-box">
            <el-row v-if="classDetail">
              <el-button @click="inform('examine', scope.row)" type="info" size="small">查看详情</el-button>
            </el-row>

            <el-row v-if="scope.row.isDeleted===0 && classStop">
              <el-button type="danger" size="small" @click="inform('stopVideo', scope.row)">停用</el-button>
            </el-row>

            <el-row v-if="scope.row.isDeleted===1 && classStop">
              <el-button type="success" size="small" @click="inform('enableVideo', scope.row)">启用</el-button>
            </el-row>

            <el-row v-if="businessId==='1'||businessId==='2'||businessId==='9'">
              <el-button type="info" size="small" @click="inform('query', scope.row)">查看导入数据</el-button>
            </el-row>

            <el-row>
              <el-button type="info" size="small" @click="inform('goInteraction', scope.row)">互动管理</el-button>
            </el-row>

            <el-row v-if="scope.row.status===3 || scope.row.status===9">
              <el-button @click="inform('getInteractData', scope.row)" type="info" size="small">互动数据</el-button>
            </el-row>

            <el-row v-if="classStatic">
              <el-button @click="inform('handleClick', scope.row)" type="info" size="small">统计信息</el-button>
            </el-row>
            ...
            >
              <el-button @click="inform('playClick', scope.row)" type="info" size="small">播放</el-button>
            </el-row>

            <el-row v-if="(scope.row.type === 2 || scope.row.status === 9)">
              <el-button type="info" size="small" @click="inform('downLoad', scope.row)">下载</el-button>
            </el-row>

            <el-row v-if="scope.row.status === 9">
              <el-button type="info" size="small" @click="inform('govideoManage', scope.row)">视频管理</el-button>
            </el-row>

            <el-row v-if="scope.row.businessId===3">
              <el-button type="info" size="small" @click="inform('userManage', scope.row)">人员管理</el-button>
            </el-row>

            <el-row v-if="businessId!=='3'">
              <el-button
                @click="inform('setContentColumns', scope.row)"
                type="info"
                size="small"
              >栏目设置</el-button>
            </el-row>
    ...
    auths() {
      switch (this.businessId) {
        ...
        case "4":
          ...
          break;
        case "5":
          if (this.userAuths.academic_conference_detail)
            this.classDetail = true;
          if (this.userAuths.academic_conference_statistics)
            this.classStatic = true;
          if (this.userAuths.academic_conference_stop_start)
            this.classStop = true;
          break;
        default:
          this.classDetail = true;
