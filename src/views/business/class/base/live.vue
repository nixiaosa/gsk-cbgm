  ...
  export default {
    ...
    methods: {
      download(type) {
        this.isLoading = true
        if (this.serviceId === 9) {
          // http.get(api.adminWatchListExport + this.busId + '/' + 1).then(res => {
          //   let data = res.data
          //   if (data.code === 0) {
        ...
        if (type === 1) {
          ...
          if (this.serviceId === 5) {
            window.location.href = api.videoWatchOut3 + this.busId + '/' + 1
          } else {
            window.location.href = api.videoWatchOut2 + this.busId + '/' + 1
          }
          this.isLoading = false
        }
      },
