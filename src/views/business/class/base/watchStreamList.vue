<template>
  <div class="list">
    <div class="user-total">
      <div>
        <span>观看流水，为用户观看视频的所有记录情况，一个用户可能多条记录数据，观众每观看1次，系统记录一次数据。</span>
      </div>
      <div class="out-data">
        <a @click="download()">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="直播观看时长（分）">
        <template slot-scope="scope">
          <span>{{ scope.row.watchVo.liveWatchedDuration | tomin}}</span>
        </template>
      </el-table-column>
      <el-table-column
        v-if="listType !=='2'"
        label="回放观看时长（分）">
        <template slot-scope="scope">
          <span>{{ scope.row.watchVo.playbackWatchedDuration | tomin}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="进入时间">
  ...
  methods: {
    ...
    getIdentityFields: async function() {
      ...
      if (res.data.code === 0) {
        ...
        var resObj3 = {
          label: "直播观看时长（分）",
          prop: "liveWatchedDuration",
          type: "viewTime2"
        };
        var resObj4 = {
          label: "回放观看时长（分）",
          prop: "playbackWatchedDuration",
          type: "viewTime2"
        };
        var resObj5 = {
          label: "进入时间",
        ...
        if(this.listType === '2'){
          this.fieldsTitleArr.push(resObj0);
        }
        if(this.listType !== '2'){
        ...
        if (this.ipLocation) {
          ...
          this.fieldsTitleArr.push(resObj10);
        }
      } else {
        this.$message.error(res.data.message);
      }
    ...
    download() {
        ...
        let downLink = this.$api.watch.watchRecordExport
    ...
    videoDataList: async function(type, pageNum) {
      ...
      if (this.listType === '2') { //如果是点播则传type,直播和回放不传type，1直播、2点播、3回放
        params1.type = type
      }
      let apiLink = this.$api.watch.watchRecord
      await http
        ...
        .then(response => {
          ...
          if(response.data.data.enrollVo){
            ...
            item.enrollFieldVos.forEach(el => {
              ...
              res["liveWatchedDuration"] = item.watchVo.liveWatchedDuration;
              res["playbackWatchedDuration"] = item.watchVo.playbackWatchedDuration;
