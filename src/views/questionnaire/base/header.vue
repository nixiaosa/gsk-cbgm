<template>
  <div>
    ...
    <div class="createBox">
      <el-form label-width="80px" :model="addoptions">
        ...
        <el-form-item label="*关联内容:">
          ...
          >{{tag.questionName?tag.questionName:tag.busName }}</el-tag>
          <el-button @click="showRelecance" type="info">选择业务内容</el-button>
        </el-form-item>
      </el-form>
    </div>
    ...
    <div class="module_box">
      ...
      <div class="module_content_box">
        ...
        <li>
          <span>每人领取</span>
          <el-input-number
            v-model="addoptions.conditions[0].pointEvery"
            :min="1"
            style="width: 160px"
          ></el-input-number>
          <span>积分</span>
    ...
    <!-- 关联内容的对话框 -->
    <el-dialog title="关联内容" :visible.sync="dialogVisible">
      <el-table
        ref="multipleTable"
      ...
      >
        ...
        <el-table-column label="时间">
          <template slot-scope="scope">{{ scope.row.createTime | formatDates }}</template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      addoptions: {
        ...
        description: "", // 简介
        event: "qa_question", // 任务表识 qa_question调研问卷
  ...
  methods: {
    getDataInfo() {
      http.get(api.questionDetails + this.taskId).then(res => {
        if (res.data.code === 0) {
    ...
    handleClose(tag) {
      this.addoptions.businesses.splice(
        this.addoptions.businesses.indexOf(tag),
        1
      );
    },
    /*
    ...
    timeChange() {
      ...
      if (this.timeScope[0].getTime() === this.timeScope[1].getTime()) {
        this.addoptions.endTime =
          this.timeScope[1].getTime() + 3600 * 1000 * 24 - 1000;
      }
    },
    /*
     * 验证填写的内容
    ...
    taskSave(status) {
      ...
      var apiurl = api.questionSave;
      if (this.taskId) {
        // apiurl = api.answerUpdate
        this.addoptions.id = this.taskId;
      }
    ...
    handleSelectionChange(item) {
      for (var i of item) {
        i.businessId = 100;
        i.id = null;
        i.relateId = i.questionId;
        if (i.questionName) {
          i.busName = i.questionName;
        }
      }
    ...
    getDateList(pageNum) {
      ...
      http.post(api.questionnaiRerelation, params).then(res => {
