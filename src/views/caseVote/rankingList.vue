  ...
  <div class="case">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      <el-form-item label="活动标题">
        <el-input v-model="caseName" placeholder="请输入活动标题"></el-input>
      </el-form-item>
    ...
    <el-table :data="tableDatas" border>
      ...
      <el-table-column label="参赛名称" width="400px">
        <template slot-scope="scope">
          <span v-if="scope.row.description">{{ scope.row.description}}</span>
          <span
            v-else-if="scope.row.caseUserName"
          >{{ scope.row.caseUserName }}、{{ scope.row.caseUserOffice }}、{{ scope.row.caseUserTitle }}、{{ scope.row.caseUserHospital }}</span>
          <span v-else>{{ scope.row.caseUserPhone}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="病例名称" width="300px">
        <template slot-scope="scope">
          <span>{{ scope.row.caseTitle }}</span>
        </template>
      </el-table-column>
      <el-table-column label="病例征集活动名称" width="300px">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    tabRes(row) {
      var optionId = row.id;
      this.$router.push({
        path: "/activity/definiteDetailed/" + optionId,
        query: {
          pages: this.newpage,
          voteId: this.$route.params.id,
          optionId: row.id,
