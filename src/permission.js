      ...
      if (store.getters.addRouters.length === 0) { // 判断异步路由是否有
        ...
        if (!store.getters.title) {
          ...
          getcompanyconf().then(result => {
            if (result) {
              store.dispatch('SetTitle', result.data.platomName + '后台管理')
              store.dispatch('SetCompanyConfig', result.data)
              store.dispatch('SetPlatimg', result.data.logoUrl)
              store.dispatch('SetPlattitle', result.data.platomName)
              // SetIpLocation
              // if (result.data.data.ip === '1') {
              //   store.dispatch('SetIpLocation', true)
              // }
              store.dispatch('SetIpLocation', result.data.ip)
              store.dispatch('SetIdentity', result.data.identity)
              store.dispatch('SetVideoAudit', result.data.videoAudit)
              store.dispatch('SetVideoStatistic', result.data.videoStatistic)
              store.dispatch('SetCaseTitleShow', result.data.caseTitleShow)
              store.dispatch('SetCaseTitleValue', result.data.caseTitleValue)
              store.dispatch('SetCmsStatistic', result.data.cmsStatistic)

              store.dispatch('SetThemeColor', result.data.themeColor)
